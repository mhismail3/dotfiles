...
- Bundled `sidebarctl.swift` (in `bin/`) for managing Finder sidebar favorites
- Finder sidebar automation in `.macos`/`finder.sh` (Home, Applications, Downloads, iCloud Drive)

### Changed
- Night Shift is now automated (removed from manual steps in `.macos` output)
- Finder settings now run through `sidebarctl`; optional .DS_Store reset and UI reload added for idempotent reruns
